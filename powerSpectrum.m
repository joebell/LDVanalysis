function powerSpectrum(varargin)
...
%     data.LDVvelocity = data.LDVvelocity((7.9*10^5):(8.1*10^5));
%     data.LDVposition = data.LDVposition((7.9*10^5):(8.1*10^5));

    L = length(data.LDVposition);
    NFFT = min([2^(nextpow2(L)),2^20]);
    chunkSize = 2^(nextpow2(L) - 5);
    overlap = .5;
...
%    subplot(2,2,2);
%    [Pxx, f] = pwelch(data.LDVvelocity,chunkSize,round(overlap*chunkSize),NFFT,data.sampleRate);  
%    loglog(f,sqrt(Pxx),'Color',plotColor); hold on;
     NFFT = 2^(nextpow2(L)-0);
     Pxx = fft(data.LDVvelocity,NFFT)./L;
     %Pxx = smooth(Pxx,3);
